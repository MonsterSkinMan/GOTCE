...
namespace GOTCE.Items.White
{
    public class SkullKey : ItemBase<SkullKey>
    {
        ...
        public override string ItemFullDescription => "Gain <style=cIsUtility>5%</style> Stage Transition Crit chance. On '<style=cIsUtility>Stage Transition Crit</style>', gain <style=cIsUtility>3</style> <style=cStack>(+3 per stack)</style> items (<style=cIsHealth>50%</style>/<style=cShrine>50%</style>).";
        ...
        public void MeWhenWithor(object sender, StageCritEventArgs args)
        {
            if (NetworkServer.active)
            {
                ...
                foreach (PlayerCharacterMasterController playerCharacterMaster in instances)
                {
                    ...
                    {
                        int maxLockboxes = playerCharacterMaster.master.inventory.GetItemCount(ItemDef) * 3;
