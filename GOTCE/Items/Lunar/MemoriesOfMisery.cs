...
namespace GOTCE.Items.Lunar
{
    public class MemoriesOfMisery : ItemBase<MemoriesOfMisery>
    {
        ...
        public override string ItemFullDescription => "Increase your <style=cIsUtility>move speed</style> by <style=cIsUtility>40%</style> <style=cStack>(+30% per stack)</style>. Invert your controls. Reduce <style=cIsHealing>maximum health</style> by <style=cIsHealing>0%</style> <style=cStack>(+15% per stack)</style>.";
        ...
        private void RecalculateStatsAPI_GetStatCoefficients(CharacterBody sender, RecalculateStatsAPI.StatHookEventArgs args)
        {
            if (sender && sender.inventory)
            {
                ...
                if (stack > 0)
                {
                    args.moveSpeedMultAdd += 0.4f + 0.3f * (stack - 1);
